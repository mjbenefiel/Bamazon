	...
	inquirer.prompt([
		{
			type: 'input',
			name: 'product_id',
			message: 'Please enter the Product ID for stock_count update.',
		...
		// Query db to confirm that the given Product ID exists and to determine the current stock_count
		var queryStr = 'SELECT * FROM products WHERE ?';

		connection.query(queryStr, {product_id: item}, function(err, data) {
			if (err) throw err;

			// If the user has selected an invalid Product ID, data attay will be empty
			...
			if (data.length === 0) {
				...
				var updateQueryStr = 'UPDATE products SET stock_quantity = ' + (productData.stock_quantity + addQuantity) + ' WHERE product_id = ' + item;
				...
				connection.query(updateQueryStr, function(err, data) {
					...
					console.log('Stock count for Product ID ' + item + ' has been updated to ' + (productData.stock_quantity + addQuantity) + '.');
					console.log("\n---------------------------------------------------------------------\n");
...
function createNewProduct() {
	...
	inquirer.prompt([
		...
		connection.query(queryStr, input, function (error, results, fields) {
			...
			console.log('New product has been added to the inventory under Product ID ' + results.insertId + '.');
			console.log("\n---------------------------------------------------------------------\n");
